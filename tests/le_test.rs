...
#[test]
fn read_write_le_mixed() {
     // LE encoded size, but BE data
     // The first 3 bytes are dummy bytes
     let buff_one: Vec<u8> = vec![32, 32, 32, 12, 0, 0, 0, 72, 101, 108, 108, 111, 32, 87, 111, 114, 108, 100, 33, 32, 32, 32, 32];
     ...
     buff_two.write_all(&length.parse()[..]).unwrap();
     // we should now have the length of 12 written in LE

     // write the contents of the string now...
     buff_two.write_all(&to_encode.as_bytes()).unwrap();

     // Write magic to buffer.
     buff_two.write_all(&[32, 32, 32, 32]).unwrap();
