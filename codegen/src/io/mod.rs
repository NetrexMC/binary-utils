...
pub(crate) type AstContext<'a> = (&'a syn::Ident, &'a Vec<syn::Attribute>, &'a syn::Generics, &'a syn::Visibility);
...
pub(crate) fn binary_encoder(input: TokenStream) -> TokenStream {
    ...
    let stream = match input.data {
        Data::Struct(d) => {
            structs::derive_struct(ctx, d, &mut err)
        },
        Data::Enum(d) => {
            enums::derive_enum(ctx, d, &mut err)
        },
        Data::Union(d) => {
            unions::derive_union(ctx, d, &mut err)
        }
    };
